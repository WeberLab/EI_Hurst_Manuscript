...
53.4 years) was performed to investigate the relationship between fMRI
...
measure of. Thus, understanding if H and E:I are correlated would
...
for E:I, it may serve as a more practical clinical biomarker for this
...
rest, while E:I (glutamate/GABA) did not change between conditions. H
and E:I were not correlated during either movie-watching or rest. This
study represents the first attempt to investigate this connection
\emph{in vivo} in humans. We conclude that, at 3T and with our
particular methodologies, no association was found. We end with lessons
learned and suggestions for future research.
...
depth review, see \citet{campbellMonofractalAnalysisFunctional2022}. In
terms of clinical findings, abnormal H values have been identified in
...
E:I-Hurst relationship \emph{in vivo}, specifically within the visual
cortex during movie-watching and resting periods. We hypothesized that H
would increase in the visual cortex during movie watching
\citep{campbellFractalBasedAnalysisFMRI2022}, and that H would
negatively correlate with E:I
\citep{trakoshisIntrinsicExcitationinhibitionImbalance}.
...
details), a further seven participants were removed for having less than
...
rest than movie, and all participants saw the same movie segment,
beginning at the same time during the scan.
...
Excitatory-inhibitory ratio (E:I) was calculated as {[}Glx from sLASER
in i.u.{]}/{[}GABA+ from MEGA-PRESS in i.u.{]}, a common practice to
report E:I using MRS \citep{rideauxNoBalanceGlutamate+glutamine2021}.
...
the \texttt{Osprey} software was reporting Glx metabolite concentrations
from the sLASER sequence that were an order of magnitude larger than
reported values in the literature (i.e.~x10). Attempts to re-analyze the
sLASER data using \texttt{fsl\_mrs}
\citep{clarkeFSLMRSEndtoendSpectroscopy2021} resulted in similarly large
...
In order to be confident in our results, however, we re-ran all MRS
analyses, acquiring quantitative Glx and GABA+ values from the
MEGA-PRESS sequence only using the software \texttt{Gannet}
\citep{eddenGannetBatchprocessingTool2014} (v3.3.0). Values were
relaxation-, tissue- and alpha-corrected using the Harris et al.~(2015)
method \citep{harrisSpectralEditingMeasurementsGABA2015}. Results using
Glx and GABA+ from the \texttt{Gannet} analysis of MEGA-PRESS only are
...
(v2023.06.0+421). Difference of means of H, Glx, and GABA+ between rest
and movie conditions were calculated using paired Student's t-tests
\citep{studentProbableErrorMean1908}. Correlations between H and Glx,
GABA+ and E:I were calculated using Pearson's method
\citep{freedmanStatistics2007}. Finally, a linear mixed-effect model was
used to test for the correlation of H and E:I while controlling for
possible confounding effects of MRS FWHM and fMRI motion (see Results).
...
After exclusion, FWHM (mean ± sd) at rest in sLASER and MEGA-PRESS were
8.52 ± 0.7 and 7.02 ± 0.7, respectively. During movie watching, FWHM in
sLASER and MEGA-PRESS were 8.37 ± 0.65 and 6.98 ± 0.86.
...
Correlation plots of H vs.~mean FD for rest (green) and movie (orange).}

\end{figure}%

\subsection{H and E:I}\label{h-and-ei}
...
in Figure~\ref{fig-results}. Neither Glx nor GABA+ were different
between movie and rest conditions. E:I ratio did not change between
conditions either. H was found to be greater during movie watching than
rest.
...
To ensure that mean FD and FWHM were not confounding our results, we
also ran a linear mixed-effects model with H as dependent variable,
Glx/GABA+ as primary predictor of interest, and mean FD, FWHM(Glx), and
FWHM(GABA+) as covariates, Condition (rest vs.~movie) as fixed effects,
...
\subsection{Re-analysis using Gannet}\label{re-analysis-using-gannet}

Re-running our analysis using Glx and GABA+ values from \texttt{Gannet}
did not result in any significant changes from the results reported
above. We did, however, obtain quantitative Glx values much closer to
what has been reported in the literature. We have included these results
...
Attempting to control for mean FD and FWHM using a linear mixed-effects
...
Indeed, Pasanta et al.~(2023) \citet{pasantaFunctionalMRSStudies2023}
found the magnitude of effect sizes were observed to be smaller for
block designs than event-related designs.
...
\item
  ...
  Pasanta et al.~(2023) \citet{pasantaFunctionalMRSStudies2023} for a
...
visualization, and wrote the original draft. JS helped with validation,
...
tissue and tissue- and alpha-correction.}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Glx} & 9.32 ± 0.85 & 9.19 ± 0.6 & 0.30 \\
\textbf{GABA+} & 3.09 ± 0.32 & 3.03 ± 0.33 & 0.22 \\
\textbf{E:I Ratio} & 3.04 ± 0.3 & 3.06 ± 0.26 & 0.68 \\
...
\includegraphics{index_files/figure-pdf/fig-gannetresults-1.pdf}
...
(orange) conditions.} A) Glx; B) GABA+; and C) E:I. Paired dots
represent the same participant across conditions. Mean difference with
95\% confidence intervals, and as Cohen's D are reported at the top of
each plot.}

\end{figure}%

H was not found to correlate with Glx, GABA+, or E:I, during rest or
movie (Figure~\ref{fig-gannetcorrelations}).
...
\includegraphics{index_files/figure-pdf/fig-gannetcorrelations-1.pdf}

}

\caption{\label{fig-gannetcorrelations}\textbf{Scatter plots of H
vs.~metabolites when analyzed using Gannet.} A) Glx; B) GABA+; and C)
E:I. Rest is in green, while movie is in orange. p-values are reported
