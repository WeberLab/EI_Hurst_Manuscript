...
| sLASER                      | 35            | 2,000      | 90         | -      | 28.0                    | 28.0 x 28.0           | -                             | 4 |
| MEGA-PRESS                  | 68            | 1,800      | 90         | -      | 28.0                    | 28.0 x 28.0           | -                             | 8 |
...
All statistical analyses were performed using `R` [@rcoreteamLanguageEnvironmentStatistical2021] and `RStudio` (v2023.06.0+421). Difference of means of H, Glx, and GABA+ between rest and movie conditions were calculated using paired Student’s t-tests [@studentProbableErrorMean1908]. Correlations between H and Glx, GABA+ and E:I were calculated using Pearson’s method [@freedmanStatistics2007]. Finally, a linear mixed-effect model was used to test for the correlation of H and E:I while controlling for possible confounding effects of MRS FWHM and fMRI motion (see Results).
...
After exclusion, FWHM (mean ± sd) at rest in sLASER and MEGA-PRESS were `r as.numeric(fwhm_stats[2,3, drop = T])` ± `r as.numeric(fwhm_stats[2,4, drop = T])`
and `r as.numeric(fwhm_stats[1,3, drop = T])` ± `r as.numeric(fwhm_stats[1,4, drop = T])`, respectively. During movie watching, FWHM in sLASER and MEGA-PRESS were `r as.numeric(fwhm_stats[4,3, drop = T])` ± `r as.numeric(fwhm_stats[4,4, drop = T])`
...
An average of all MRS voxel placements can be seen in @fig-mrsquality A, and a sample of the `Osprey` sLASER and MEGA-PRESS spectrum fits at rest can be seen in @fig-mrsquality B and C, respectively.
...
To ensure that mean FD and FWHM were not confounding our results, we also ran a linear mixed-effects model with H as dependent variable, Glx/GABA+ as primary predictor of interest, and mean FD, FWHM(Glx), and FWHM(GABA+) as covariates, Condition (rest vs. movie) as fixed effects, and subjects as random effects:
...
Re-running our analysis using Glx and GABA+ values from `Gannet` did not result in any significant changes from the results reported above. We did, however, obtain quantitative Glx values much closer to what has been reported in the literature. We have included these results in the Supplementary Material.

# Discussion

We report here the first *in vivo* human study of the E:I-H relationship. An increase in H was observed during movie-watching as compared to rest, indicating stronger long-range temporal dependencies in BOLD activity during visual stimulation. No difference was found in Glx, GABA+, nor Glx/GABA+ between rest and movie-watching. Furthermore, no association was found between H and Glx, GABA+, nor Glx/GABA+. Attempting to control for mean FD and FWHM using a linear mixed-effects model did not alter these results.
...
Indeed, Pasanta et al. (2023) @pasantaFunctionalMRSStudies2023 found the magnitude of effect sizes were observed to be smaller for block designs than event-related designs.
...
JS helped with validation, visualization, and reviewed and edited the manuscript.
...
|                  | Rest | Movie | p-value |
| ---------------- | ---- | ----- | ------- |
| **Glx**    | `r as.numeric(glu_stats[1,2, drop = T])` ± `r as.numeric(glu_stats[1,3, drop = T])` | `r as.numeric(glu_stats[2,2, drop = T])` ± `r as.numeric(glu_stats[2,3, drop = T])`  | `r format_p(ttest_glu$p.value)` |
| **GABA+**  | `r as.numeric(gaba_stats[1,2, drop = T])` ± `r as.numeric(gaba_stats[1,3, drop = T])` | `r as.numeric(gaba_stats[2,2, drop = T])` ± `r as.numeric(gaba_stats[2,3, drop = T])`  | `r format_p(ttest_gaba$p.value)` |
| **E:I Ratio**    | `r as.numeric(ei_stats[1,2, drop = T])` ± `r as.numeric(ei_stats[1,3, drop = T])` | `r as.numeric(ei_stats[2,2, drop = T])` ± `r as.numeric(ei_stats[2,3, drop = T])`  | `r format_p(ttest_ei$p.value)` |
...
#| fig-cap: "**Paired comparison of metabolite values analyzed using Gannet during rest (green) and movie (orange) conditions.** A) Glx; B) GABA+; and C) E:I. Paired dots represent the same participant across conditions. Mean difference with 95% confidence intervals, and as Cohen's D are reported at the top of each plot."
#| fig-width: 10
#| fig-height: 8
layout <- "
  AABB
...
H was not found to correlate with Glx, GABA+, or E:I, during rest or movie (@fig-gannetcorrelations).
<!-- TODO: add r to the above sentence -->


```{r}
#| label: fig-gannetcorrelations
#| fig-cap: "**Scatter plots of H vs. metabolites when analyzed using Gannet.** A) Glx; B) GABA+; and C) E:I. Rest is in green, while movie is in orange. p-values are reported at the top of each plot."
#| fig-width: 7
#| fig-height: 6
layout <- "
AABB
