...
`r capitalize_first(as.character(english(as.numeric(count(orig_subjects)))))` healthy adult participants (`r sum(orig_subjects$Sex == "M")` M, age `r round(min(orig_subjects$Age),1)` -- `r round(max(orig_subjects$Age),1)`) took part in the study. After our analysis and performing quality assurance (see below), `r as.character(english(as.numeric(count(orig_subjects) - count(subjects))))` participants were removed for having less than ideal MRI data quality, leaving `r as.character(english(as.numeric(count(subjects))))` final participants, between the ages of `r round(min(subjects$Age),1)` and `r round(max(subjects$Age),1)` (mean age ± sd: `r round(mean(subjects$Age),1)` ± `r round(sd(subjects$Age),1)` years; `r sum(subjects$Sex == "M")` males).
...
The following MRI scans were acquired. A 3D T1-weighted sagittal fast spoiled gradient echo (FSPGR) sequence:  TE = 2.176 ms, TR = 7.216 ms, flip angle = 12, FOV = 256 x 256, slice thickness = 0.9 mm, and in-plane resolution = 0.9375 x 0.9375 mm^2^.
A 3D T2-weighted sagittal CUBE (fast spin echo): TE = 75.242 ms, TR = 2,504.0 ms, slice thickness = 0.9 mm, and in-plane resolution = 0.9375 x 0.9375 mm^2^.
A 2D echo-planar imaging (EPI) multi-echo gradient-echo fMRI sequence: 3 echoes, TE1 = 12.2 ms, TE2 = 35.352 ms, TE3 = 58.504 ms, TR = 1,500 ms, flip angle = 52, acceleration factor=6, in-plane resolution = 3.5938 x 3.5938 mm^2^, and slice thickness = 3.6 mm.
A semi-LASER (sLASER) sequence: TE = 35 ms, TR = 2,000 ms, flip angle = 90, and FOV = 512 x 512.
A MEGA-PRESS sequence: TE = 68 ms, TR = 1,800 ms, flip angle = 90, and FOV = 512 x 512.
...
Since it cannot be assumed that all fBm is removed from the signal, we use the concept of ‘extended Hurst’ (H’) in this study: for 0 < H < 1, the signal is understood as fGn, while for 1 < H < 2, the signal is assumed to be fBm [@campbellFractalBasedAnalysisFMRI2022]. More generally, it is also assumed that when 0.5 < H < 1.5, the signal displays 1/f behaviour [@zimmernWhyBrainCriticality2020].H was calculated for all voxels in the brain of each subject. A brain mask was then applied which included only GM and the region of the MRS voxel in the visual cortex (see @fig-fmrigreyvoxel). H was averaged across the brain mask area, using only non-zero voxels.

{{< embed notebooks/Figures.ipynb#fig-fmrigreyvoxel >}}

## Statistics

All statistical analyses were performed using `R` [@rcoreteamLanguageEnvironmentStatistical2021] and `RStudio` (v2023.06.0+421). Glutamate, GABA, E/I ratio, and H were compared between rest and movie conditions using paired Student’s t-tests [@studentProbableErrorMean1908]. Glutamate, GABA, and E/I ratio were tested for correlation with HE using Pearson’s method [@freedmanStatistics2007].
...
An average of all MRS voxel placements can be seen in @fig-mrsquality A. 

<!-- {{< embed notebooks/Figures.ipynb#fig-mrsvoxel >}} -->
