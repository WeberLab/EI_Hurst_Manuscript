...
Average water FWHM, creatine SNR, and frequency shift are reported in Supplementary Material @supptbl-quality.

Average voxel rotation along the x-axis, along with average tissue fractions are reported in Supplementary Material @supptbl-singlevoxel.
...
Diagnostics of our linear-mixed effects model can be found in the Supplementary Materials (@suppfig-lmemdiag; @supptbl-spearmanresults; and @supptbl-robusteffects)
...
Mean FD was not correlated with H during rest (r = `r round(meanfd_rest.model$estimate,2)`, p = `r round(meanfd_rest.model$p.value,2)`) but was moderately negatively correlated with H during movie watching (r = `r format(round(meanfd_movie.model$estimate,2), nsmall=2)`, p = `r round(meanfd_movie.model$p.value,2)`; see Supplementary Materials @suppfig-dataquality A).

Glx and GABA+ were tested for associations with FWHM values during rest and movie watching. No significant correlations were found (see Supplementary Materials @suppfig-dataquality B and C). Glx and GABA+ were tested for associations with frequency shift during rest and movie watching. No significant correlations were found (see Supplementary Materials @suppfig-dataquality D and E).

Water FWHM and creatine SNR (CR-SNR) for both sLASER and MEGA-PRESS were analyzed pre- vs. post-fMRI. Supplementary @supptbl-FWHMprepost, @supptbl-CRSNR, and @supptbl-FWHMCRSNR present the paired t-tests and subject-level rest/movie values, while Supplementary @suppfig-fwhmdriftplot show the individual trajectories. No systematic drifts were detected (all p > 0.2 for FWHM; p > 0.3 for SNR).
<!-- TODO: fix table and figure reference -->
Furthermore, a group averaged plot of the frequency shift between rest and movie can be found in the Supplementary Materials (@suppfig-frequencyshift).
...
| Trakoshis et al. (2020)@trakoshisIntrinsicExcitationinhibitionImbalance | Computational with simulated data; in vivo in mice | fMRI BOLD signal | Wavelet-based maximum likelihood method | E-to-I synaptic conductance | Negative linear |
...
- future studies should consider using ultra-high-field 7T MRI, which provides improved spectral resolution and more reliable detection of Glu and GABA compared to conventional 3T MRI [@terpstraTestretestReproducibilityNeurochemical2016]; 
...
A summary of voxel placement statistics are provided in @supptbl-singlevoxel.
...
**Summary of spectroscopy voxel placement values** GM = grey matter; WM = white matter; CSF = cerebrospinal fluid.
:::
...
We report Spearman correlations between E:I and H within each condition (@supptbl-spearmanresults). Spearman rho values were modest and negative (Movie: $\rho$ = `r round(spearman_results$spearman_ei_hurst[1],2)`, p = `r round(spearman_results$p_value[1],2)`; Rest: $\rho$ = `r round(spearman_results$spearman_ei_hurst[2],2)`, p = `r round(spearman_results$p_value[2],2)`), and consistent with the primary analyses in direction but not statistically significant.

::: {#supptbl-spearmanresults}
```{r}
knitr::kable(spearman_results, digits = 3)
```
Spearman correlation between EI and Hurst by condition.
:::

We also refit the linear mixed-effects model after removing the top/bottom 10% of observations in E:I, in H, and in the intersection of both (@supptbl-robusteffects). Across these trimmed datasets, the E:I fixed-effect estimate remained small and its 95% confidence interval continued to include zero, indicating that the primary conclusion is not driven by extreme values in either E:I or H. Trimming was performed on the pooled long-format dataset (rest and movie rows) to match the model specification.

Taken together with the non-parametric Spearman correlations (@supptbl-spearmanresults), these analyses indicate that the E:I–H association is weak in this pilot dataset and that our primary inference (a small effect with confidence intervals spanning zero) is unchanged under non-parametric estimation and reasonable exclusion of potential outliers.

::: {#supptbl-robusteffects}

```{r}
knitr::kable(robust_effects, digits = 3)
```
EI fixed-effect estimates (Estimate ± 95% CI) for the full sample and each trimmed sample.
:::
...
All metabolite concentrations and Cramer-Rao lower bounds from semi-LASER are reported in @supptbl-metabolites.
...
```
**All metabolite concentrations and Cramer-Rao lower bounds from semi-LASER.** Concentration values are in millimoles; Cramer-Rao lower bounds are in percentages. Conc = concentration; CRLB = Cramer-Rao lower bounds.
:::

### MEGA-PRESS {.unnumbered}

All metabolite concentrations and Cramer-Rao lower bounds from J-edited MEGA-PRESS are reported in @supptbl-megapressmetabs.

::: {#supptbl-megapressmetabs}
```{r}
#| tbl-cap: ""
...
```
**All metabolite concentrations and Cramer-Rao lower bounds from J-edited MEGA-PRESS.** Concentration values are in millimoles; Cramer-Rao lower bounds are in percentages. Conc = concentration; CRLB = Cramer-Rao lower bounds.
::: 
...
Data quality correlations for Hurst with mean framewise displacement, Glx and GABA+ with FWHM, and Glx and GABA+ with frequency shift are shown in @suppfig-dataquality.
...
free(meanfd_hurst_plot)+ free(glxandgabaplus_FWHM_plot) + free(glxandgabaplus_freqshift_plot) +
  ...
  plot_annotation(tag_levels = 'A') & theme(plot.tag = element_text(size = 11))
```
**Data quality correlations.** A) Correlation plots of H vs. mean FD (mm) for rest (green) and movie (orange). B) and C) Correlation plots of Glx (B) and GABA+ (C) vs. water FWHM (Hz) for rest (green) and movie (orange). D) and E) Correlation plots of Glx (B) and GABA+ (C) vs. frequency shift (ppm) for rest (green) and movie (orange).
:::

Rest vs movie FWHM comparisons (paired t-tests) are provided in @supptbl-FWHMprepost.

::: {#supptbl-FWHMprepost}
```{r}
names(fwhm_tests) <- c("Seq", "m. rest", "m. movie", "m. diff", "sd diff", "t", "p", "CIlow", "CIhigh")
knitr::kable(fwhm_tests, digits = 2)
```
Rest vs. movie FWHM comparison (paired t-tests) for each spectroscopy sequence. Seq = sequence; m. = mean; sd = standard deviation; t = t-value; p = p-value; CIlow = lower confidence interval; CIhigh = higher confidence interval.
:::

Rest vs movie FWHM comparisons (paired t-tests) are provided in @supptbl-CRSNR.

::: {#supptbl-CRSNR}
```{r}
names(snr_tests) <- c("Seq", "m. rest", "m. movie", "m. diff", "sd diff", "t", "p", "CIlow", "CIhigh")
knitr::kable(snr_tests, digits = 2)
```
Rest vs. movie CR-SNR comparison (paired t-tests) for each spectroscopy sequence. Seq = sequence; m. = mean; sd = standard deviation; t = t-value; p = p-value; CIlow = lower confidence interval; CIhigh = higher confidence interval.
:::

Subject-level FWHM and CR-SNR values for each sequence (Rest, Movie, and Movie–Rest difference) are provided in @supptbl-FWHMCRSNR.

::: {#supptbl-FWHMCRSNR}
```{r}
names(drift_table) <- c("Sub", "Seq", "R FWHM", "M FWHM", "Diff FWHM", "R SNR", "M SNR", "Diff SNR")
knitr::kable(drift_table, digits = 2)
```
Subject-level FWHM and CR-SNR values for each sequence (Rest, Movie, and Movie–Rest difference). Sub = Subject; Seq = Sequence; R = Rest; M = Movie; Diff = Difference.
:::

Plots of individual water FWHM changes between rest and movie-watching conditions for both MEGA-PRESS and sLASER are shown in @suppfig-fwhmdriftplot.

::: {#suppfig-fwhmdriftplot}
```{r}
#| fig-width: 5
#| fig-height: 5
fwhm_drift_plot
```
Plots of individual water full-width half-maximum changes between rest and movie-watching conditions for both MEGA-PRESS and sLASER
:::

Frequency shift between rest and movie are shown in @suppfig-frequencyshift.

::: {#suppfig-frequencyshift}
```{r}
#| fig-width: 5
#| fig-height: 7
...
p1 + plot_annotation(tag_levels = "A") + theme(plot.tag = element_text(size = 11))
```
**Frequency shift between rest and movie.** Overlay of all participants' frequency drift (ppm) plots during rest (blue) and movie (red) for A) semi-LASER and B) MEGA-PRESS. Note, the GE semi-LASER sequence averaged all transients (128) into groups of 4, each with 32 transients, before being saved as raw P-file.
:::
...
Mean ± sd of metabolites and E:I when analyzed using Gannet, during rest and movie, are reported in @supptbl-gannetresults. Boxplots between rest and movie can be seen in @suppfig-gannetresults. Neither Glx nor GABA+ were different between movie and rest conditions. E:I ratio did not change between conditions either.

::: {#supptbl-gannetresults}
|                  | Rest | Movie | p-value |
| ---------------- | ---- | ----- | ------- |
...
**Summary of main metabolite results using Gannet** 
:::

::: {#suppfig-gannetresults}
```{r}
#| fig-width: 10
#| fig-height: 8
layout <- "
...
```
**Scatter plots of H vs. metabolites when analyzed using Gannet.** A) Glx (mM); B) GABA+ (mM); and C) E:I. Rest is in green, while movie is in orange. P-values are reported at the top of each plot.
:::

Glx estimates were not strongly correlated between sLASER and MEGA-PRESS (Rest $r = `r round(glx_agreement_table$pearson_r[glx_agreement_table$Condition == "Rest"], 2)`$, Movie $r = `r round(glx_agreement_table$pearson_r[glx_agreement_table$Condition == "Movie"], 2)`$), while mean absolute differences remained under `r round(max(abs(glx_agreement_table$mean_difference)),2)` institutional units. @supptbl-glxagreement reports the paired summary statistics, and @suppfig-glxagreementplot visualises the subject-level agreement.

This finding is consistent with several studies that have directly compared Glx measured from MEGA-PRESS "OFF" spectra, with 'gold standard' short-TE sequences (PRESS and sLASER), which have consistently reported poor agreement [@vanveenendaalGlutamateQuantificationPRESS2018; @bellVivoGlxGlu2021; @thomsonVivoGlxMeasurements2026]. Reasons for this include the fact that MEGA-PRESS uses a longer TE time (68 ms compared to sLASER's 35 ms) [@maddockEstimatingGlutamateGlx2018]; chemical shift displacement error differences between the two sequences [@archibaldTestretestReliabilityMultimetabolite2025]; and scanner drift due to MEGA-PRESS being acquired on average `r round(avg_rest/60,2)` ± `r round(sd_rest/60,2)` (Rest) and `r round(avg_movie/60,2)` ± `r round(sd_movie/60,2)` minutes (Movie) after sLASER.

::: {#supptbl-glxagreement}
```{r}
names(glx_agreement_table) <- c("Cond", "n", "m. sLASER", "m. MEGA", "m. diff", "sd diff", "m.a. diff", "r", "p", "CIlow", "CIhigh")
knitr::kable(glx_agreement_table, digits = 2)
```
Agreement between sLASER- and MEGA-PRESS-derived Glx within each condition. Cond = condition; n = number of subjects; m. = mean; m.a. = mean absolute; r = Pearson correlation; p = p-value; CIlow = lower confidence interval; CIhigh = higher confidence interval.
:::

::: {#suppfig-glxagreementplot}

```{r}
glx_agreement_plot
```
Glx from MEGA-PRESS plotted against Glx from sLASER, during both rest (green) and movie-watching (orange). Dashed line represents perfect agreement.
:::
